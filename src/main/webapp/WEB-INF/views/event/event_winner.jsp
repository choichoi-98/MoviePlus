			...
			<div class="inner-wrap">
				<div class="location">
					<span>Home</span> <a
						...
						href="${pageContext.request.contextPath}/event/winner"
						title="당첨자발표 페이지로 이동">당첨자발표</a>
				</div>
		...
		<div id="contents">
			...
			<div class="inner-wrap">
				...
				<div class="text-util">
					...
					<div class="right">
						...
						<sec:authorize access="isAnonymous()">
							<a href="#" id="myeventbtn" class="button"
								title="나의 이벤트 응모내역 페이지로 이동">나의 응모결과확인</a>
						</sec:authorize>

						<!-- 로그인 후 -->
						<sec:authorize access="isAuthenticated()">
							<a href="${pageContext.request.contextPath}/member/myevent"
								class="button" title="나의 이벤트 응모내역 페이지로 이동">나의 응모결과확인</a>
						</sec:authorize>
					</div>
				</div>
				...
				<c:if test="${eventlistcount > 0}">
					<div class="board-list-util">
						<p class="result-count">
							<strong>전체 ${eventlistcount}건</strong>
						</p>
							...
							<tbody>
								<c:forEach var="event" items="${eventlist}">
									<tr>
										...
										<!-- 이벤트 타입 -->
										<th scope="row"><a
											href="${pageContext.request.contextPath}/event/detail?num=${event.EVENT_NUM}"
											data-no="14078" data-netfunnel="N" class="eventBtn" title="">
												${event.EVENT_SUBJECT}</a> <!-- 이벤트명 --></th>
										<td><span class="resultdate">${event.EVENT_RESULTDATE}</span></td>
										<!-- 발표일 -->
										<!-- 당첨자발표 현황(준비중/결과확인) -->
										<td>
											<!-- <button type="button" class="button gray x-small w70px" disabled="disabled">준비중</button>  -->
											<a
											href="${pageContext.request.contextPath}/event/winnerdetail?num=${event.EVENT_NUM}"
											class="button x-small black-line w70px"
											title="당첨자발표 결과확인 상세보기">결과확인</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
				...
				<nav class="pagination">
					<a title="처음 페이지 보기" href="winner?page=1"
						class="control first">first</a> <a title="이전 페이지 보기"
						href="winner?page=${page-1}" class="control prev"
					...
					<c:forEach var="a" begin="${startpage}" end="${endpage}">
						...
						<c:if test="${a != page }">
							<a title="${a} 페이지보기" href="winner?page=${a}">${a}</a>
						</c:if>
					</c:forEach>

					<a title="이후 페이지 보기" href="winner?page=${page+1}"
						class="control next"
						${page >= maxpage ? 'style="pointer-events: none;"' : ''}>next</a>

					<a title="마지막 페이지 보기" href="winner?page=${maxpage}"
