...
<body>
	...
	$(document).ready(function() {
		$('#searchBtn').click(function() {
			location.href = 'regionnotice?keyword=' + $('#searchTxt').val();
		})
	})
	</script>
	...
	<div class="container has-lnb">
		...
		<div id="contents" class="" style="margin-left: 22%; float: none;">
			...
			<nav class="pagination" style="float: left; margin-left: 25%">
				<a title="처음 페이지 보기" href="regionnotice?page=1&keyword=${keyword}" class="control first">first</a>
				<a title="이전 페이지 보기" href="regionnotice?page=${page-1}&keyword=${keyword}"
					class="control prev"
				...
				<c:forEach var="a" begin="${startpage}" end="${endpage}">
					...
					<c:if test="${a != page }">
						<a title="${a} 페이지보기" href="regionnotice?page=${a}&keyword=${keyword}">${a}</a>
					</c:if>
				</c:forEach>

				<a title="이후 페이지 보기" href="regionnotice?page=${page+1}&keyword=${keyword}"
					class="control next"
					${page >= maxpage ? 'style="pointer-events: none;"' : ''}>next</a>

				<a title="마지막 페이지 보기" href="regionnotice?page=${maxpage}&keyword=${keyword}"
